...
GIMP_LOCALES_DIR = os.path.join(SCRIPT_DIR, '../workspace/gimp/po')
OUTPUT_JSON_PATH = os.path.join(SCRIPT_DIR, '../generated/gimp.json')

def po_listup():
	poFilePaths = []
	for p in os.listdir(GIMP_LOCALES_DIR):
		c = p.split('.')
		if len(c) == 2 and c[-1] == 'po':
			file_path = os.path.join(GIMP_LOCALES_DIR, p)
